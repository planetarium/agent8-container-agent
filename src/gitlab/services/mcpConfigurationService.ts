import type { McpToolMetadata, McpTransferData, McpServerConfigExtended } from "../../types/mcpMetadata.js";
...
export class McpConfigurationService {
   ...
   */
  async formatMcpConfiguration(
    projectId: number,
    issueIid: number
  ): Promise<string | null> {
    console.log(`[MCP-Debug] Starting formatMcpConfiguration for project ${projectId}, issue #${issueIid}`);
    ...
    try {
      ...
      console.log(`[MCP-Debug] Successfully retrieved MCP data for issue #${issueIid}, formatting...`);
      const configValue = encodeURIComponent(JSON.stringify(mcpData.servers));
      const formattedConfig = `mcpServers=${configValue}`;

      console.log(`[MCP-Debug] Formatted MCP config for issue #${issueIid}: ${formattedConfig.substring(0, 100)}...`);
  ...
  async retrieveMcpConfiguration(
    ...
    console.log(`[MCP-Debug] Starting retrieveMcpConfiguration for project ${projectId}, issue #${issueIid}`);

    try {
      console.log(`[MCP-Debug] Fetching internal notes for project ${projectId}, issue #${issueIid}...`);
      const internalNotes = await this.gitlabClient.getInternalNotes(
        projectId,
        issueIid
      );

      console.log(`[MCP-Debug] Retrieved ${internalNotes.length} internal notes for issue #${issueIid}`);
      ...
      internalNotes.forEach((note, index) => {
        console.log(`[MCP-Debug] Internal note ${index + 1}/${internalNotes.length} for issue #${issueIid}:`);
        ...
        console.log(`[MCP-Debug]   - Contains MCP marker: ${note.body.includes(this.METADATA_MARKER)}`);
        console.log(`[MCP-Debug]   - Body preview: ${note.body.substring(0, 200)}...`);
      });

      for (const note of internalNotes) {
        if (note.body.includes(this.METADATA_MARKER)) {
          console.log(`[MCP-Debug] Found MCP metadata marker in note ${note.id} for issue #${issueIid}`);
          const result = this.deserializeAndTransformConfiguration(note.body);

          if (result) {
            console.log(`[MCP-Debug] Successfully parsed MCP configuration for issue #${issueIid}:`, result);
            return result;
          } else {
            console.log(`[MCP-Debug] Failed to parse MCP configuration from note ${note.id} for issue #${issueIid}`);
          }
        }
      }

      console.log(`[MCP-Debug] No MCP configuration marker found in any internal notes for issue #${issueIid}`);
      return null;
    } catch (error) {
  ...
  async hasMcpConfiguration(projectId: number, issueIid: number): Promise<boolean> {
    console.log(`[MCP-Debug] Checking if MCP configuration exists for project ${projectId}, issue #${issueIid}`);
  ...
  private deserializeAndTransformConfiguration(noteBody: string): McpTransferData | null {
    ...
    const regex = new RegExp(
      `${this.METADATA_MARKER}\\s*([\\s\\S]*?)\\s*${this.METADATA_MARKER}`
    );
