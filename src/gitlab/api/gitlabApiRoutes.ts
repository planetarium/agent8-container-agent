...
export class GitLabApiRoutes {
  ...
  async handleRequest(req: Request): Promise<Response | null> {
    ...
    try {
      ...
      if (path.startsWith('/api/gitlab/issues/') && method === 'GET') {
        const issueId = parseInt(path.split('/')[4]);
        return await this.handleIssueDetail(issueId, corsHeaders);
      }

      if (path.match(/^\/api\/containers\/[^\/]+\/gitlab-issue$/) && method === 'GET') {
  ...
  private isGitLabApiPath(path: string): boolean {
    return path.startsWith('/api/gitlab') ||
           path.match(/^\/api\/containers\/[^\/]+\/gitlab-issue$/) !== null;
  }
  ...
  private async handleIssueDetail(issueId: number, corsHeaders: Record<string, string>): Promise<Response> {
  ...
  private async handleContainerIssue(containerId: string, corsHeaders: Record<string, string>): Promise<Response> {
